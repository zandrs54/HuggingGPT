...
export default class ChatBot {
     ...
     */
    async readCookiesFromPath(path) {
        if (!path)
     ...
     */
    async chat(text, currentConversionID, temperature = 0.1, truncate = 1000, max_new_tokens = 1024, top_p = 0.95, repetition_penalty = 1.2, top_k = 50, return_full_text = false, stream = true, use_cache = false, is_retry = false) {
        if (text == "")
     ...
     */
    async summarizeConversation(conversation_id) {
        if (!conversation_id) {
     ...
     */
    async preserveContext(newChat) {
        let response;
        if (newChat) {
            response = await fetch("https://huggingface.co/chat/conversation/" +
                this.currentConversionID +
